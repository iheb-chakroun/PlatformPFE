...
public class PfefichierService {
	...
	@Path("all")
	@Produces("application/json")
	public Response getAllpfefilesall(@QueryParam("id") int id, @QueryParam("year") String year,
			@QueryParam("r") Role r) {
	...
	@Path("list")
	@Produces("application/json")
	public Response getAllprevalidatedfiles(@QueryParam("id") int id, @QueryParam("r") String r) {

		if (PfeFichierBusiness.getAllPfeFilencadreur(id, Role.valueOf(r)) == null)
			return Response.status(Response.Status.NOT_FOUND).build();

		if (PfeFichierBusiness.getAllPfeFilencadreur(id, Role.valueOf(r)).size() == 0)
			return Response.status(Response.Status.NO_CONTENT).entity("Pas de contenu").build();

		else
			return Response
					.ok(PfeFichierBusiness.getAllPfeFilencadreur(id, Role.valueOf(r)), MediaType.APPLICATION_JSON)
					.build();
